...
// Fetch and store weather data for a specific location (using lat/lon)
const fetchAndStoreWeatherDataByCoords = async (lat, lon) => {
  ...
  try {
    ...
    const weatherData = {
      city: data.name, // Use the city name returned by the API
      date: new Date().toISOString().split('T')[0],  // current date in YYYY-MM-DD format
      temperature: {
        min: data.main.temp_min,
        max: data.main.temp_max
...
// Endpoint to fetch weather data for the current location using latitude and longitude
app.get('/api/weather/current', async (req, res) => {
  ...
  try {
    ...
    await fetchAndStoreWeatherDataByCoords(lat, lon);

    // Retrieve and return the weather data from MongoDB
    const weatherData = await Weather.findOne({ date: new Date().toISOString().split('T')[0] }).sort({ date: -1 });
    res.json(weatherData);
  } catch (err) {
    console.error('Error fetching weather data:', err.message);
    res.status(500).json({ error: 'Error fetching weather data' });
